...
// check required
const checkRequired = ( value: string, errors: {[fieldName: string]: {error: string}}, fieldName: string ): void => {
  if (value) {
...
const checkNumber = ( value: string, errors: {[fieldName: string]: {error: string}}, fieldName: string ): void => {
...
const checkNegative = ( value: string, errors: {[fieldName: string]: {error: string}}, fieldName: string ): void => {
  if (+value < 0) {
    errors[fieldName].error += ERROR_MSG.NUMBER;
  } else {
    errors[fieldName].error += "";
