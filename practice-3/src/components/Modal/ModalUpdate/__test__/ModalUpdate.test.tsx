...
describe("Modal create component", () => {
  ...
  const setup = () => {
    const utils = render(<ModalUpdate product={PRODUCT_MOCKING}
  ...
  test("should change file image when onChang input", async () => {
    const { getByTestId, queryByTestId } = render(<ModalUpdate product={PRODUCT_MOCKING}
      hideModalUpdate={() => {}} isReload={true} setIsReLoad={() => {}} />);
    ...
    await waitFor(() => expect(queryByTestId("after-change-file")).toBeTruthy());
  });
  ...
  test("should update product when click Submit", () => {
    const { getByTestId } = render(<ModalUpdate product={PRODUCT_MOCKING}
      hideModalUpdate={() => {}} isReload={true} setIsReLoad={updateProduct} />);
